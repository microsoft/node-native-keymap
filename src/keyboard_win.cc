...
NAN_METHOD(GetCurrentKeyboardLayout) {
  ...
  Nan::Set(result, Nan::New("name").ToLocalChecked(),
      Nan::New<v8::String>(layout_name.data(), layout_name.length()).ToLocalChecked());
  Nan::Set(result, Nan::New("id").ToLocalChecked()),
      Nan::New<v8::String>(layout_id.data(), layout_id.length()).ToLocalChecked();
  Nan::Set(result, Nan::New("text").ToLocalChecked(),
      Nan::New<v8::String>(layout_text.data(), layout_text.length()).ToLocalChecked());

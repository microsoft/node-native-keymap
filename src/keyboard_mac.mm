...
void OnKeyboardChangeNotificationReceived(CFNotificationCenterRef center,
                                          void* observer,
                                          CFStringRef name,
                                          const void* object,
...
NAN_METHOD(GetKeyMap) {
  ...
  for (size_t i = 0; i < cnt; ++i) {
    ...
    auto withShiftAltGr = ConvertKeyCodeToText(keyboardLayout, native_keycode, kShiftKeyModifierMask | kAltKeyModifierMask);
    Nan::Set(entry, Nan::New("withShiftAltGr").ToLocalChecked(),
        Nan::New<v8::String>(withShiftAltGr.second.data(), withShiftAltGr.second.length()).ToLocalChecked());
...
NAN_METHOD(OnDidChangeKeyboardLayout) {
  ...
  CFNotificationCenterAddObserver(
      CFNotificationCenterGetDistributedCenter(), nullptr,
